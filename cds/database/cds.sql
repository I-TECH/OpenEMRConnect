...
MySQL - 5.1.45-community : Database - cds
*********************************************************************
*/
...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`cds` /*!40100 DEFAULT CHARACTER SET utf8 */;
...
CREATE TABLE `cda` (
  ...
  `birthdate` date COMMENT 'date of birth following MOH guidance',
...
DROP TABLE IF EXISTS `cda_to_process`;

CREATE TABLE `cda_to_process` (
  `cda_id` int(11) NOT NULL,
  CONSTRAINT `fk_cda_id` FOREIGN KEY (`cda_id`) REFERENCES cda(`cda_id`) ON UPDATE CASCADE ON DELETE CASCADE,
  UNIQUE (`cda_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
